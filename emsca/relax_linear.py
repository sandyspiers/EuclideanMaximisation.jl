...
# Copied from
# https://github.com/IBMDecisionOptimization/docplex/blob/953ef184a7005baa77256ea7c02db116e13ecd6c/docplex/mp/relax_linear.py
# Code has been updated to
#   1. Ensure the return type is a CplexModel, and NOT the type of given model.
...
class LinearRelaxer(object):
    ...
    def linear_relaxation(
        self, mdl, return_partial=False, **kwargs
    ) -> tuple[CplexModel, dict]:
        ...
        try:
            relaxed_model.objective_expr = mdl.objective_expr.relaxed_copy(
        ...
        if copy_parameters:
            ...
            nb_copied = 0
            for p1, p2 in zip(
                mdl.parameters.generate_params(),
                relaxed_model.parameters.generate_params(),
            ):
                if p1.is_nondefault():
                    p2.set(p1.get())
                    nb_copied += 1
            if verbose:
                info(
                    "copied {0} initial model parameters to relaxed model".format(
                        nb_copied
                    )
                )
        ...
        if relax_sos:
            for sos in mdl.iter_sos():
                ...
                relaxed_model.add(
                    relaxed_model.sum_vars(sos_vars) <= sos_type.value, sos_ctname
                )
        else:
            for sos in mdl.iter_sos():
        ...
        if unrelaxables:
            ...
            if verbose and not return_partial:
                print(
                    "* model {0}: found {1} un-relaxable elements, main cause is {2} (e.g. {3})".format(
                        mdl.name, nb_unrelaxables, main_cause, justifier
                    )
                )
            if verbose:
                for cause, urxs in unrelaxables.items():
                    ...
                    for u, urx in enumerate(urxs):
                        if hasattr(urx, "is_generated") and urx.is_generated():
                            ...
                            s_gen = ""
                        print(
                            "--  {0}: cannot be relaxed: {1!s}{2}".format(
                                u + 1, urx, s_gen
                            )
                        )
            if return_partial:
                ...
                return relaxed_model, var_mapping
            else:
                return None
        else:
            # force cplex if any...
